...
> =>
  Effect.gen(function* () {
    ...
    const initialResume = options.resumeCursor ?? parseAntigravityResume(options.resumeCursor);
          ...
          try {
                    ...
                    yield* Deferred.await(answersDeferred).pipe(
                      Effect.flatMap((resolvedAnswers) =>
                        Effect.gen(function* () {
                          ...
                          const resEventId = yield* makeEventId(
                            "user-input.resolved",
                            currentTurnId,
                          );
            ...
            if (parsed.event === "result" && parsed.result) {
              ...
              if (currentTurnId && currentItemId && activeTurn) {
                ...
                yield* Ref.update(turnsRef, (turns) => [
                  ...turns,
                  {
                    id: currentTurnId,
            ...
            // Not a JSON line, fall through to raw line handling
          }
        }

        // Plain text fallback
        if (currentTurnId && currentItemId && activeTurn) {
          activeTurn.fullText += line + "\n";
          const eventId = yield* makeEventId("content.delta", currentTurnId);
          const createdAt = yield* nowIso;
          yield* offerEvent({
            eventId,
            ...
            createdAt,
            type: "content.delta",
            payload: { streamKind: "assistant_text", delta: line + "\n" },
          });
        }
      });

...
// Lightweight chats live outside the work inbox: the Conversations
// pseudo-project never renders as a project row, its threads render here as
// a flat shelf under the project list. The header stays put so a new
// conversation is always one click away.
function SidebarConversationsSection({
  conversationThreads,
  ...
  conversationThreads: readonly SidebarThreadSummary[];
  ...
  const orderedThreadKeys = useMemo(
    () => conversationThreads.map((t) => scopedThreadKey(scopeThreadRef(t.environmentId, t.id))),
    [conversationThreads],
  );
  ...
  const commitRename = useCallback(
    ...
    [cancelRename, updateThreadMetadata],
  );
  ...
  const handleThreadContextMenu = useCallback(
    async (threadRef: ScopedThreadRef, position: { x: number; y: number }) => {
      ...
      const clicked = await api.contextMenu.show(
        [
          ...
          { id: "delete", label: "Delete conversation", destructive: true, icon: "trash" },
        ],
        position,
      );

      if (clicked === "rename") {
        startThreadRename(scopedThreadKey(threadRef), thread.title);
    ...
    [attemptArchiveThread, deleteThread, startThreadRename],
  );
  ...
  const handleMultiSelectContextMenu = useCallback(
    async (position: { x: number; y: number }) => {
      ...
      const count = selectedThreadKeys.length;
      if (count === 0) return;

      const items = buildMultiSelectThreadContextMenuItems({ count, hasRunningThread: false });
      const clicked = await api.contextMenu.show(items, position);
      if (clicked === "archive") {
        for (const key of selectedThreadKeys) {
          const ref = parseScopedThreadKey(key);
          if (ref) await archiveThread(ref);
        }
        clearSelection();
      }
    },
    [archiveThread, clearSelection],
  );

  return (
    <SidebarGroup className="px-2 py-2">
      <div className="mb-1 flex items-center justify-between pl-2 pr-1.5">
        <span className="text-xs font-medium text-sidebar-muted-foreground/80">Conversations</span>
        <Tooltip>
          <TooltipTrigger
            render={
              <button
      ...
      {conversationThreads.length > 0 ? (
        ...
        <div className="px-2 pt-2 text-xs text-sidebar-muted-foreground/75">
...
export default function LegacySidebar() {
  ...
  // The Conversations pseudo-project renders as its own shelf below the
  // project list, never as a scoped project row.
  const realSidebarProjects = useMemo(
    () => sidebarProjects.filter((project) => !isConversationsProject(project)),
    [sidebarProjects],
  );

  const sidebarProjectByKey = useMemo(
    () => new Map(realSidebarProjects.map((project) => [project.projectKey, project] as const)),
    [realSidebarProjects],
  );
  const sidebarThreadByKey = useMemo(
    () =>
  ...
  const conversationThreads = useMemo(() => {
    const conversationsProject = sidebarProjects.find((project) => isConversationsProject(project));
    if (!conversationsProject) return [];
    return sortThreads(
      (threadsByProjectKey.get(conversationsProject.projectKey) ?? []).filter(
        (thread) => thread.archivedAt === null,
      ),
      sidebarThreadSortOrder,
    );
  }, [sidebarProjects, sidebarThreadSortOrder, threadsByProjectKey]);
  ...
  const handleProjectDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      const activeProject = realSidebarProjects.find((project) => project.projectKey === active.id);
      const overProject = realSidebarProjects.find((project) => project.projectKey === over.id);
    ...
    [orderedProjects, sidebarProjectSortOrder, reorderProjects, realSidebarProjects],
  );
  ...
  const sortedProjects = useMemo(() => {
    const sortableProjects = realSidebarProjects.map((project) => ({
      ...project,
